...
OldFont = ''
FONTS_TO_CONVERT = {
    'Fulfulde - Aissata': {'encoding': 'Aissata', 'replace': 'Noto Sans Adlam'},
    'Fulfulde - Pulaar': {'encoding': 'Pulaar', 'replace': 'Noto Sans Adlam'},
    }
...
def convertOneDoc(path_to_doc, unicodeFont='RibengUni',
  ...
  (numConverts, numNotConverted) = convertDoc(doc, unicodeFont, debugInfo=debugFlag)
...
def processArgs(argv):
  if len(sys.argv) <= 1:
    print ('Usage:')
    print ('  convertWordAdlam.py inputFile.docx')
    print ('  convertWordAdlam.py inputFile1.docx inputFile2.docx ... ')
    print ('  convertWordAdlam.py -i fileWithFileNames')
    return None
